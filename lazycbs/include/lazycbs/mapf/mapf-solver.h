...
class MAPF_Solver {
  ...
  // encode a (row, column) location with lazycbs integer encoding
  ...
  // compare a (row, column) location with lazycbs integer encoded location
...
//   bool minimizeMakespan();
  ...
  vec<mapf::patom_t> createAssumption(int agent, tuple<int, int> locations, int time, int cost, bool forbidden);
